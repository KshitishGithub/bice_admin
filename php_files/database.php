...
class Database
{
    ...
    public function sendsms($header, $smsid, $value = [], $mobile = [])
    {
        ...
        $this->select('setting', 'sms_status');
        $sms_status = $this->getResult();
        if ($sms_status[0]['sms_status'] == 1) { // IF SMS Status is active
            ...
            array_push($this->result, "success"); // success but sms not sending..
        }
    }
